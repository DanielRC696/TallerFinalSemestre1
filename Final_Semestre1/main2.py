...
def generate_pdf(results, filename="clientes_report.pdf"):
    c = canvas.Canvas(filename, pagesize=letter)
    width, height = letter  # Tamaño de la página A4 (letter)

    # Posición de inicio en el PDF
    y_position = height - 40  # Empezamos cerca de la parte superior de la página

    # Título
    c.setFont("Helvetica-Bold", 14)
    c.drawString(200, y_position, "Reporte de Clientes")
    y_position -= 30  # Movemos la posición hacia abajo

    # Formato de tabla
    c.setFont("Helvetica", 10)

    # Encabezados de columna
    for i, header in enumerate(results[0][0]):
        c.drawString(50 + (i * 100), y_position, header)

    y_position -= 15  # Baja la posición para empezar las filas

    # Filas de los datos
    for row in results[0][1]:
        for i, cell in enumerate(row):
            c.drawString(50 + (i * 100), y_position, str(cell))
        y_position -= 15  # Baja la posición para la siguiente fila

        # Si se llega al final de la página, añadimos una nueva página
        if y_position < 40:
            c.showPage()  # Añadir una nueva página
            c.setFont("Helvetica", 10)
            y_position = height - 40  # Reiniciamos la posición Y
...
class ClientApp(tk.Tk):
    ...
    def show_all_clients(self):
        ...
        if results:
            self.display_results(results)  # Mostrar resultados en la interfaz
